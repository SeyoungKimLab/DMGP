...
import dmgp
from dmgp.model import TMGP
from dmgp.core import vec, vec_inv, KP, ind_kmeans

gpflow.config.set_default_jitter(1e-6)

# CONSTANTS
TRAIN_PARAMS_PATH = './params/train_tmgp_params.yaml'
...
# Trains TMGP on a given dataset
def train_tmgp(train_params, main_log_dir, group_idxs=None, input_idxs=None, show_summary=False, exp_id=None):
    ...
    # Instantiate model
    model = TMGP(Y_train.shape, train_ids, kernel=SquaredExponential(), ll=Gaussian(), ind_points=inds)
    ...
    def inference_TMGP(data, ids, split):
        ...
        if n * p < 10000:
            ...
            f_mean_X_fixed, f_mean_X_random, f_mean_G_fixed = f_means 
            f_var_X_fixed, f_var_X_random, f_var_G_fixed = f_vars
        ...
        # Default: Use mini-batch settings for training
            ...
            for idx_split in tqdm(idx_splits):
                batch, batch_ids = get_batch(data, (np.array(range(p)), np.array(range(n))), idx_split)
                b_prime = batch[1].shape[0]
                a_prime = batch[0].shape[0]
                f_means_batch, f_vars_batch = model.predict_f(batch, batch_ids, sep='fixed')
                f_mean_X_fixed_batch, f_mean_X_random_batch, f_mean_G_fixed_batch = f_means_batch
                f_var_X_fixed_batch, f_var_X_random_batch, f_var_G_fixed_batch = f_vars_batch
    ...
    inference_TMGP(train_data, train_ids, 'train')
    inference_TMGP(test_data, test_ids, 'test')
...
def main(**kwargs):
    ...
    main_log_dir = './logs/train_tmgp_logs'
    ...
    print(f'Loaded TMGP train parameter settings.')

    if n_exps == -1:
        train_tmgp(train_params, main_log_dir, show_summary=show_summary)
    else:
        for exp_id in range(1,n_exps+1):
            print(f'Running experiment {exp_id}...')
            train_tmgp(train_params, main_log_dir, show_summary=show_summary, exp_id=exp_id)
